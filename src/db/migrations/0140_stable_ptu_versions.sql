...
-- Old PTU data is superseded — just delete data from non-canonical (non-max-id) rows,
-- then delete the rows themselves. This avoids UNIQUE(uuid, game_version_id) conflicts
-- that would occur if we tried to re-point data.
--
-- Collect old PTU/EPTU IDs: any row where a newer row exists for the same channel.
-- Safe if 0 or 1 PTU rows exist (DELETE affects 0 rows).

-- Delete data from old PTU/EPTU version IDs across all versioned tables.
-- Child tables first (FK ordering), then parent tables.
...
-- Now delete from all versioned tables
