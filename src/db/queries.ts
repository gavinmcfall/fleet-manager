  ...
  const result = await db
    .prepare(
      ...
      SELECT
        ...
        CASE WHEN child.name IS NOT NULL THEN child.name ELSE vc.name END AS component_name,
        CASE WHEN child.type IS NOT NULL THEN child.type ELSE vc.type END AS component_type,
