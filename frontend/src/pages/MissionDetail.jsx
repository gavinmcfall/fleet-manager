...
function RepBar({ tiers }) {
  const maxRank = 6
  return (
    <div className="relative h-2 rounded-full bg-white/[0.04] overflow-hidden">
      {tiers.map((tier, i) => {
        const left = (tier.min_rank / maxRank) * 100
        const nextMin = tiers[i + 1]?.min_rank ?? maxRank
        const width = ((nextMin - tier.min_rank) / maxRank) * 100
        const color = DIFF_COLORS[tier.difficulty] || 'text-gray-400'
        const bgColor = color.replace('text-', 'bg-').replace('/400', '/30')
        return (
          <div
            key={tier.difficulty}
            className={`absolute top-0 h-full ${bgColor} transition-all`}
            style={{ left: `${left}%`, width: `${width}%` }}
            title={`${DIFF_LABELS[tier.difficulty] || tier.difficulty}: Rank ${tier.min_rank}+`}
          />
        )
      })}
    </div>
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="relative bg-white/[0.03] backdrop-blur-md border border-white/[0.06] rounded-2xl p-6 overflow-hidden">
        ...
        <div className="flex items-start gap-5">
          {logo && <img src={logo} alt="" className="w-16 h-16 rounded-lg border border-white/[0.08] object-cover shrink-0" />}
          <div className="flex-1 min-w-0">
            ...
            {generator.description && (
              <p className="text-sm text-gray-500 leading-relaxed">{generator.description}</p>
            )}

            {/* Faction details */}
            {(generator.headquarters || generator.leadership) && (
              <div className="flex flex-wrap gap-x-5 gap-y-1 mt-3 text-xs text-gray-500">
                {generator.headquarters && (
                  <span className="flex items-center gap-1.5"><Building2 className="w-3 h-3 text-gray-600" /> {generator.headquarters}</span>
                )}
                {generator.leadership && (
                  <span className="flex items-center gap-1.5"><Users className="w-3 h-3 text-gray-600" /> Led by {generator.leadership}</span>
                )}
              </div>
            )}
          </div>
      ...
      {systems.length > 0 && (
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-xs uppercase tracking-wider text-gray-600">Available in</span>
          {systems.map(sys => (
      ...
      {tiers && tiers.length > 0 && (
        <div>
          <h2 className="text-xs uppercase tracking-wider text-gray-500 mb-3 flex items-center gap-2">
            <Shield className="w-3.5 h-3.5" /> Difficulty Tiers
          </h2>

          {/* Rep progression bar */}
          <RepBar tiers={tiers} />
          <div className="flex justify-between mt-1 mb-4 text-[10px] text-gray-600">
            <span>Rank 0</span>
            <span>Rank 6</span>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/[0.08] bg-white/[0.02]">
                  <th className="text-left text-[10px] uppercase tracking-wider text-gray-500 font-medium px-4 py-2.5">Difficulty</th>
                  <th className="text-left text-[10px] uppercase tracking-wider text-gray-500 font-medium px-4 py-2.5">Min Rep</th>
              ...
              <tbody>
                {tiers.map(tier => (
                  <tr key={tier.difficulty} className="border-b border-white/[0.04] last:border-0 hover:bg-white/[0.02] transition-colors">
                    <td className={`px-4 py-2 text-xs font-medium ${DIFF_COLORS[tier.difficulty] || 'text-gray-400'}`}>
                    ...
                    <td className="px-4 py-2 text-xs text-gray-400 font-mono">
                      {tier.rep_reward ? `+${tier.rep_reward}` : '—'}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {all_blueprints.length > 0 && (
            <div className="flex items-start gap-2 mt-3 px-3 py-2 rounded-lg bg-sc-accent/5 border border-sc-accent/10">
              ...
              <p className="text-xs text-gray-400">
                <span className="text-sc-accent font-medium">Blueprint drops are the same at all difficulty levels.</span> Higher difficulty increases aUEC and reputation rewards, not the blueprint loot pool.
              </p>
            </div>
          )}
        </div>
